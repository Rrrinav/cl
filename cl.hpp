...
#define CL_DEBUG_L1(fmt, ...) do { if constexpr (CL_DEBUG_LEVEL >= 1) std::println(stderr, "[CL:L1] " fmt __VA_OPT__(,) __VA_ARGS__); } while(0)
#define CL_DEBUG_L2(fmt, ...) do { if constexpr (CL_DEBUG_LEVEL >= 2) std::println(stderr, "[CL:L2] " fmt __VA_OPT__(,) __VA_ARGS__); } while(0)
#define CL_DEBUG_L3(fmt, ...) do { if constexpr (CL_DEBUG_LEVEL >= 3) std::println(stderr, "[CL:L3] " fmt __VA_OPT__(,) __VA_ARGS__); } while(0)

#define CL_ASSERT(expr, fmt, ...) \
    do {\
        if (!(expr)) {\
            std::println(stderr, "ASSERTION FAILED: {}", std::format(fmt __VA_OPT__(,) __VA_ARGS__));\
...
#define CL_ASSERT_LOC(loc, expr, fmt, ...) \
    do {\
        if (!(expr)) {\
            std::println(stderr, "{}:{}: [ERR]: {}", loc.file_name(), loc.line(), std::format(fmt __VA_OPT__(,) __VA_ARGS__));\
            std::exit(EXIT_FAILURE);\
        }\
    } while(false)
...
template<typename T>
concept Parsable_Type_C = Supported_Scalar_C<T> || 
                          (is_std_array_v<T> && Supported_Scalar_C<typename T::value_type>);

// Types we can RETRIEVE via get<> (Scalars, Arrays, and Vectors)
template<typename T>
concept Gettable_Type_C = Parsable_Type_C<T> || 
                          (is_std_vector_v<T> && Supported_Scalar_C<typename T::value_type>);
...
struct Opt
{
    ...
    {
        default_val_ = {};  // zero-init whole array
        std::size_t i = 0;
...
// Modified to accept Vectors (recursively checks value_type)
template<typename T>
requires Gettable_Type_C<T>
[[nodiscard]]
consteval Opt_type type_to_opt_type()
{
    if constexpr (is_std_array_v<T>)                         return type_to_opt_type<typename T::value_type>();
    if constexpr (is_std_vector_v<T>)                        return type_to_opt_type<typename T::value_type>();
...
template<Opt_type T> struct _opt_type_to_type_t_ {};
template<> struct _opt_type_to_type_t_<Opt_type::Int>   { using value = long long; };
template<> struct _opt_type_to_type_t_<Opt_type::Bool>  { using value = bool; };
template<> struct _opt_type_to_type_t_<Opt_type::Str>   { using value = std::string; }; 
template<> struct _opt_type_to_type_t_<Opt_type::Float> { using value = double; };
...
{
    uint8_t value_binding = P_SPACE | P_EQUAL;                    
...
public:
    struct Option
    {
        ...
        std::function<std::expected<void, std::string>(void*)> validate_fn;
        std::function<std::expected<void, std::string>(void*, const std::vector<std::string_view>&)> parse_action;
...
public:
    ...
    requires Gettable_Type_C<T>
    [[nodiscard]]
    auto get(Opt_id id) const -> const T&
    {
        CL_ASSERT(id < options_.size(), "Invalid Option ID: {}", id);
        Option* o = options_[id];
        
        constexpr Opt_type enum_type = type_to_opt_type<T>();
        CL_ASSERT(o->type == enum_type, "Type mismatch in get<T> for option id {}", id);
        
        // This is safe because 'add' guarantees storage is CanonicalElem
...
private:
    template <typename... Args>
    [[noreturn]]
    inline auto panic(std::source_location loc, std::format_string<Args...> fmt, Args&&... args) -> void
    {
    ...
    static auto string_to_value(std::string_view s) -> std::expected<Dest, std::string>
    {
        if constexpr (std::is_same_v<Dest, std::string> || std::is_same_v<Dest, std::string_view>) {
            ...
            if (ec != std::errc{}) return std::unexpected(std::format("Invalid integer: '{}'", s));
            return v;
        }
        else if constexpr (std::is_floating_point_v<Dest>) {
            Dest v{};
            auto [p, ec] = std::from_chars(s.data(), s.data() + s.size(), v);
            if (ec != std::errc{}) return std::unexpected(std::format("Invalid float: '{}'", s));
            return v;
        }
    }
};
...
struct std::formatter<cl::Parser::Option, char>
{
    ...
    {
        auto out = ctx.out();
        std::format_to(out, "Option {{ id: {}, type: {}, flags: 0b{:b} }}", o.id, o.type, o.flags);
        return out;
    }
};
...
{
    CL_DEBUG_L1("ADD option: {}", opt.args[0].empty() ? opt.args[1] : opt.args[0]);
    ...
    constexpr Opt_type target_enum = type_to_opt_type<T>();
    using CanonElem = typename _opt_type_to_type_t_<target_enum>::value;
    ...
    // Auto-set flag bit for bools (using trait directly for constexpr strictness)
    if constexpr (std::is_same_v<T, bool>) {
        if (!(opt.flags_ & (F_FLAG | F_EXPLICIT_BOOL))) opt.flags_ |= F_FLAG;
    }

    std::size_t arity = 1;
    if constexpr (is_std_array_v<T>) arity = std::tuple_size_v<T>;
    ...
    if (is_multi) {
        // Always store vector<CanonElem> for multi
        ...
        if constexpr (is_std_array_v<T>) {
            ...
            // Convert default array T to array CanonElem
            ...
            // Explicit cast from user default (e.g. int) to storage (e.g. long long)
    ...
    // 4. Construct Parse Action (Parsing directly into CanonElem)
    auto parse_action_lambda = [val_ptr, is_multi, arity](void* storage, const std::vector<std::string_view>& raw_tokens) -> std::expected<void, std::string> 
    {
        if (is_multi) {
            // is_multi => T is scalar, storage is vector<CanonElem>
            auto* vec = static_cast<std::vector<CanonElem>*>(storage);
            for (auto sv : raw_tokens) {
                // Parse directly to Canonical Type
                auto res = Parser::string_to_value<CanonElem>(sv); 
                if (!res) return std::unexpected(res.error());
                vec->push_back(*res);
            }
        } 
        else {
            // Not Multi - Array or Scalar
            if constexpr (is_std_array_v<T>) {
                constexpr size_t N = std::tuple_size_v<T>;
                auto* arr = static_cast<std::array<CanonElem, N>*>(storage);
                
                if (raw_tokens.size() != arity) return std::unexpected(std::format("Expected {} elements, got {}", arity, raw_tokens.size()));
                
                for (size_t i=0; i<arity; ++i) {
                    auto res = Parser::string_to_value<CanonElem>(raw_tokens[i]);
                    if (!res) return std::unexpected(res.error());
                    (*arr)[i] = *res;
                }
            } 
                ...
                if (raw_tokens.empty()) {
                    // For flags (bool), empty token means true
                    if constexpr (std::is_same_v<CanonElem, bool>) { *sc = true; return {}; }
    ...
    // 5. Construct Validation Action (Bridging Canon -> T)
    auto validate_action_lambda = [val_ptr, is_multi, validators = opt.validators_](void* storage) -> std::expected<void, std::string>
    {
        if (validators.empty()) return {};
        
        if constexpr (is_std_array_v<T>) {
             // Arrays cannot be multi (asserted earlier).
             constexpr size_t N = std::tuple_size_v<T>;
             auto* canon_arr = static_cast<std::array<CanonElem, N>*>(storage);
             
             T user_arr;
             for(size_t i=0; i<N; ++i) {
                 user_arr[i] = static_cast<typename T::value_type>((*canon_arr)[i]);
             }
            ...
            // T is Scalar. It might be multi (vector<Canon>) or single (Canon).
            if (is_multi) {
                auto* vec = static_cast<std::vector<CanonElem>*>(storage);
                for (const auto& item : *vec) {
                    // T must be scalar here due to !is_std_array_v check
                    T user_val = static_cast<T>(item); 
                    for (const auto& v : validators) {
                        auto r = (*v)(user_val);
                        if (!r) return r;
                    }
    ...
    Option* o = arena_.make<Option>(Option{
        ...
        .value = val_ptr,
        .validate_fn = validate_action_lambda,
        .parse_action = parse_action_lambda,
        .default_val_str = default_val_str_cache,
        .validator_helps = v_helps
    });

    options_.push_back(o);
    runtime_.push_back(Runtime{}); // Empty runtime state
    return id;
}
...
auto cl::Parser::parse(int argc, char *argv[]) -> std::expected<void, std::string>
{
    ...
    while (!args.empty())
    {
        ...
        if (tok.starts_with("--")) {
            ...
            if (auto eq = body.find('='); eq != std::string_view::npos) {
                if (!(cfg_.value_binding & P_EQUAL)) return std::unexpected("Equals syntax not allowed");
                key = body.substr(0, eq);
                explicit_val = body.substr(eq + 1);
            }

            if (!long_arg_to_id_.contains(key)) return std::unexpected(std::format("Unknown option: --{}", key));
            auto id = long_arg_to_id_[key];
            auto* opt = options_[id];
            auto& rt = runtime_[id];

            rt.seen = true;
            CL_DEBUG_L2("Found long option: --{}", key);
            ...
            // Values
            if (explicit_val) {
                // Check if array and comma split needed
                ...
                // If MULTI, we usually consume 1 unless it's a multi-array (forbidden).
                ...
                while (needed > 0) {
                    ...
                    // Simple heuristic: if starts with -, it's a flag, unless it's a number.
                    if (next->starts_with("-")) {
                        double d;
                        auto [p, ec] = std::from_chars(next->data(), next->data()+next->size(), d);
                        if (ec != std::errc{}) break; // Not a number, so it's a flag
                    }
                    
                    std::string_view val = *args.pop();
                    CL_DEBUG_L3("Consuming value: {}", val);
        ...
        if (tok.starts_with("-") && tok.size() > 1) {
            ...
            while (!body.empty()) {
                ...
                if (!short_arg_to_id_.contains(key)) return std::unexpected(std::format("Unknown option: -{}", key));
                
                auto id = short_arg_to_id_[key];
                auto* opt = options_[id];
                auto& rt = runtime_[id];
                rt.seen = true;
                CL_DEBUG_L2("Found short option: -{}", key);

                body = body.substr(1); // consume key
                ...
                if (opt->arity == 0) {
                    ...
                    // 2. Next arg?
                        ...
                        while(needed > 0) {
                             auto next = args.peek();
                             if (!next || next->starts_with("-")) break; // heuristic
                             rt.tokens.push_back(*args.pop());
                             needed--;
                        }
                    }
                }
    ...
    // 2. Finalization (Lazy Parsing & Validation)
    CL_DEBUG_L1("{}", "Finalizing options...");
    for (auto* opt : options_) {
        ...
        // If seen, Parse & Validate
        if (rt.seen) {
            if (!(opt->flags & F_MULTI)) {
                if (opt->arity > 0 && rt.tokens.empty()) return std::unexpected(std::format("Option {} requires value", opt->names[0]));
            }

            // Execute Parse
            auto res = opt->parse_action(opt->value, rt.tokens);
            if (!res) return res;

            // Execute Validate
            auto val_res = opt->validate_fn(opt->value);
            if (!val_res) return val_res;
        }
    }
    CL_DEBUG_L1("{}", "Parse successful");
    return {};
}
...
auto cl::Parser::print_help(std::ostream& os) -> void
{
    ...
    for (const auto* opt : options_) {
        ...
        size_t w = 0;
        if (!opt->names[1].empty()) w += 2 + opt->names[1].size(); // -s
        if (!opt->names[1].empty() && !opt->names[0].empty()) w += 2; // , 
        if (!opt->names[0].empty()) w += 2 + opt->names[0].size(); // --long
        if (!opt->meta.empty()) w += 1 + opt->meta.size(); //  <ARG>
    ...
    for (const auto* opt : options_) {
        ...
        // Extra info
        if (opt->flags & F_REQUIRED) os << " [Required]";
        if (!opt->default_val_str.empty() && opt->arity > 0) os << " [Default: " << opt->default_val_str << "]";
